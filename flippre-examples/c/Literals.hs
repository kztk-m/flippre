...
-- Int Printer
pprInt :: (Phased s) => FliPprM s v (In v IntLit -> Exp s v D)
pprInt = do
...
pprFloat :: (Phased s) => FliPprM s v (In v FloatLit -> Exp s v D)
pprFloat = do
...
pprLit :: (Phased s) => FliPprM s v (In v Literal -> Exp s v D)
pprLit = do
  int <- pprInt
  float <- pprFloat

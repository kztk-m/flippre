...
pprAssignmentOp :: (Phased s) => FliPprM s v (In v AssignmentOp -> F.Exp s v D)
pprAssignmentOp =
  share $ \x ->
    case_
      x
...
pprUnaryOp :: (Phased s) => FliPprM s v (In v UnaryOp -> F.Exp s v D)
pprUnaryOp = share $ \x ->
  case_
    x
    [ unAddress $ text "&"
...
pprExp :: (Phased s) => (In v TypeName -> F.Exp s v D) -> FliPprM s v (In v Exp -> F.Exp s v D, In v Exp -> F.Exp s v D, In v Exp -> F.Exp s v D)
pprExp pTypeName = F.do
  lit <- pprLit

...
--   EMRec :: Container2 f => f (RecI f exp (E arg exp)) -> (f exp -> E arg exp r) -> E arg exp r
...
-- compile (EVar e)    = e 
-- compile (EMRec f k)  =
--   runC (ffix (fmap2 (\r -> Rec $ \x -> compile $ runRec r x) f) ) (compile . k)
  
-- --  ffix (mapApps compile . f) (compile . k)

data FliPpr t = FliPpr (forall arg exp. FliPprC arg exp => exp t)

flippr :: (forall arg exp. FliPprC arg exp => E exp t) -> FliPpr t
flippr x = FliPpr (unE x) -- (compileR (fmap ERec x >>= id))
...
nonrec :: FliPprC arg exp => E exp t -> C exp (E exp t)
nonrec e = E <$> flet (unE e)
...
example1 = flippr $ unC $ do
  ...
  pprTF <- nonrec $ arg $ \i -> manyParens $ case_ i

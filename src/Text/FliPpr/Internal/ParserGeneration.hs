...
instance ParserLike p => FliPprC PArg (PExp p) where
  fapp :: forall a t. In a => PExp p (a :~> t) -> PArg a -> PExp p t 
  fapp (PExp f) (PArg n) =
    PExp $ \tenv -> 
  ...
  farg :: forall a t. In a => (PArg a -> PExp p t) -> PExp p (a :~> t)
  farg f = PExp $ \tenv -> 

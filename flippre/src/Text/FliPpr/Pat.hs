...
class Decomp s v a r t | r -> s v t, s v a t -> r where
  decomp :: In v a -> r -> Exp s v t

instance Decomp s v () (Exp s v t) t where
  decomp = ununit

instance (Decomp s v as r t) => Decomp s v (a, as) (In v a -> r) t where
  decomp as f = unpair as $ \a1 a2 -> decomp a2 $ f a1

-- | 'br' combines 'Branch', 'pat' and 'decomp'.
--
--   prop> br p k = Branch (pat p) $ \a -> decomp a k
br :: (Decomp s v b r t) => PatT () b a -> r -> Branch (In v) (Exp s v) a t
